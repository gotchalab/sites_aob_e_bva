...
var inscricoes = new List<TransportExcelExporter.InscricaoRow>
{
    new(
        ...
        NumAvesConcurso: 5, NumAvesVenda: 0, NumAvesTransporte: 0,
        SocioBva: "Sócio",
    ...
    new(
        ...
        Pais: "Portugal", LocalRecolha: "Vilela",
        NumAvesConcurso: 8, NumAvesVenda: 4, NumAvesTransporte: 2,
    ...
    new(
        ...
        NumAvesConcurso: 8, NumAvesVenda: 0, NumAvesTransporte: 3,
        SocioBva: "Não sócio",
...
Console.WriteLine($"  {"Nome",-18} {"Sócio",-20} {"TOTAL",8} {"BVA Port.",10} {"BVA Mstr.",10}");
for (int r = 2; r <= 4; r++)
{
    Console.WriteLine($"  {ins.Cell(r,3).GetString(),-18} {ins.Cell(r,11).GetString(),-20} " +
                      $"{ins.Cell(r,18).GetDouble(),8:F2} " +
                      $"{ins.Cell(r,19).GetDouble(),10:F2} " +
                      $"{ins.Cell(r,20).GetDouble(),10:F2}");
}
Console.WriteLine($"  {"TOTAL",-18} {"",-20} " +
                  $"{ins.Cell(5,18).GetDouble(),8:F2} " +
                  $"{ins.Cell(5,19).GetDouble(),10:F2} " +
                  $"{ins.Cell(5,20).GetDouble(),10:F2}");

Console.WriteLine();
Console.WriteLine("── Valores esperados (regra do formulário) ─────────────────");
static void Expected(string nome, int nC, int nV, int nT, string estatuto)
{
...
Expected("João Sócio",      5, 0, 0, "JaSocio");
Expected("Maria PagaAgora", 8, 4, 2, "PagaComInscricao");
Expected("Pedro NãoSócio",  8, 0, 3, "NaoSocio");
