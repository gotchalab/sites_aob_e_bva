      ...
      {hasCover ? (
        <>
          ...
          <div className="flex flex-1 flex-col gap-1.5 p-4">
            ...
            {article.publishedAt && (
              <div className="mt-auto text-xs text-ink-500">{formatDate(article.publishedAt)}</div>
            )}
          </div>
        </>
      ) : (
        <div className="relative flex h-full flex-col justify-between overflow-hidden bg-gradient-to-br from-sand-100 via-sand-200 to-brand-100 p-4">
          ...
          <div className="relative mt-6 flex flex-1 flex-col">
            <h3 className="line-clamp-4 font-display text-xl font-bold leading-tight tracking-tight text-ink-900 group-hover:text-brand-600">
              {article.title}
            </h3>
            {article.excerpt && (
              <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-ink-800/75">
                {article.excerpt}
              </p>
            )}
            {article.publishedAt && (
              <div className="mt-auto pt-3 text-xs text-ink-500">{formatDate(article.publishedAt)}</div>
            )}
          </div>
        </div>
      )}
