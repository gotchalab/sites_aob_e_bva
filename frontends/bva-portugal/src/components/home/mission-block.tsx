...
export function MissionBlock({ home }: { home: HomeConfig }) {
  ...
  const highlights = [
    ...
    home.speciesCount ? { label: "Mutações reconhecidas", value: String(home.speciesCount) } : null,
        ...
        <div className="grid gap-12 md:grid-cols-[1fr,1fr] md:items-start md:gap-16">
          <div>
            ...
            <h2 className="font-display text-3xl font-semibold leading-[1.15] tracking-tight text-ink-900 md:text-[2.5rem]">
              {home.missionTitle ?? "A associação técnica portuguesa de Agapornis."}
            </h2>
    ...
    <aside className="md:pt-4">
      <div className="border-l-2 border-brand-500 pl-6 md:pl-8">
        ...
        <p className="mt-5 font-display text-xl leading-snug text-ink-900 md:text-2xl">
          {home.missionQuote ?? "Décadas dedicadas à criação técnica de Agapornis — promovendo standards, exposições e a partilha entre criadores em Portugal."}
        </p>
      </div>
        ...
        <dl className="mt-10 grid grid-cols-2 gap-x-6 gap-y-8 md:pl-8">
          {highlights.map((h) => (
            <div key={h.label}>
              ...
              <dd className="mt-1 font-display text-3xl font-semibold text-ink-900">{h.value}</dd>
            </div>
          ))}
        </dl>
      )}
