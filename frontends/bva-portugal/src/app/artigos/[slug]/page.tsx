    ...
    <>
      ...
      <section className="hero-pattern border-b border-sand-300">
        <div className="mx-auto max-w-3xl px-4 pt-10 pb-8 md:pt-14">
          ...
          <h1 className="mt-4 font-display text-3xl font-bold leading-tight text-ink-900 md:text-5xl">
            {article.title}
          </h1>
      ...
      {article.coverImagePath && (
        <div className="mx-auto mt-8 flex max-w-4xl justify-center px-4">
          <CoverImage
            ...
            alt={article.title}
            className="max-h-[70vh] w-auto max-w-full rounded-2xl object-contain shadow-xl ring-1 ring-ink-900/5"
            hideOnError
          />
        </div>
      )}
      ...
      <article className="mx-auto max-w-3xl px-4 py-10">
        ...
        {article.tags && article.tags.length > 0 && (
          <div className="mt-10 flex flex-wrap gap-2 border-t border-sand-300 pt-6">
            {article.tags.map((t) => (
              <span
                key={t}
                className="rounded-full border border-sand-300 bg-sand-100 px-3 py-1 text-xs font-medium text-ink-700"
              >
                #{t}
              </span>
            ))}
        ...
        <div className="mt-10 flex items-center justify-between border-t border-sand-300 pt-6">
          <Link
            href="/artigos"
