    ...
    <>
      <section className="hero-pattern border-b border-sand-300">
        <div className="mx-auto max-w-6xl px-4 py-10 md:py-14">
          <h1 className="font-display text-3xl font-bold text-ink-900 md:text-4xl">Artigos</h1>
          <p className="mt-2 text-sm text-ink-500">{list.total} resultados</p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        <div className="flex flex-col gap-8 lg:flex-row">
          <aside className="w-full flex-shrink-0 lg:w-64">
            ...
            <ul className="space-y-1 rounded-lg border border-sand-300 bg-white p-2">
              <li>
                <Link
                  href="/artigos"
                  className={`block rounded-md px-3 py-2 text-sm transition ${
                    !sp.category
                      ...
                      : "text-ink-800 hover:bg-sand-100 hover:text-brand-600"
                  }`}
                >
                  Todas
                </Link>
              ...
              {categories.map((c) => (
                <li key={c.id}>
                  ...
                  {c.children.length > 0 && (
                    <ul className="ml-3 mt-1 space-y-1 border-l border-sand-300 pl-2">
                      {c.children.map((cc) => (
                        <li key={cc.id}>
                          <Link
                            ...
                            className={`block rounded-md px-2 py-1 text-xs transition ${
                              sp.category === cc.slug
                                ? "bg-brand-500 text-white"
                                : "text-ink-700/80 hover:bg-sand-100 hover:text-brand-600"
                            }`}
                          >
                            {cc.name}
                          </Link>
          ...
          <section className="flex-1 min-w-0">
            {list.items.length === 0 ? (
              <p className="text-ink-500">Nenhum artigo encontrado.</p>
            ) : (
              <LoadMoreArticles
