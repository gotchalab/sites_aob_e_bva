...
/**
 * Remove a primeira <img> do conteudo se apontar para a mesma URL do cover.
 * Evita mostrar a imagem duas vezes (banner + primeira do content) quando o
 * editor a inseriu tambem no CKEditor.
 */
...
export function cleanExcerpt(raw: string | null | undefined, maxLen = 220): string {
  if (!raw) return "";
  let s = raw;
  s = s.replace(/\{[^{}]*\}/g, ""); // strip Joomla shortcodes like {phocadownload view=file|id=7}
