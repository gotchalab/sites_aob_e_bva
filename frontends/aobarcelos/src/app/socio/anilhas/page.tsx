    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Histórico de pedidos e respetivo estado.</p>
        </div>
      ...
      {pedidos.length === 0 ? (
        <p className="mt-6 text-slate-500">Sem pedidos.</p>
      ) : (
            ...
            <li key={p.id} className="flex flex-col gap-2 p-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="text-sm font-semibold">
                  {p.especieCientifica}
                  {p.especieNomeComum && <span className="ml-2 text-slate-500">({p.especieNomeComum})</span>}
                </div>
                <div className="text-xs text-slate-500">
                  {p.ano} · Ø {p.diametro} mm · {p.quantidade} un · Pedido em {new Date(p.dataPedido).toLocaleDateString("pt-PT")}
                  {p.dataEntrega && ` · Entregue em ${new Date(p.dataEntrega).toLocaleDateString("pt-PT")}`}
                </div>
                {p.observacoes && <div className="mt-1 text-xs text-slate-500">Obs: {p.observacoes}</div>}
              </div>
              <span className={`self-start rounded px-2 py-1 text-xs font-medium ${estadoBadge[p.estado] ?? "bg-slate-100 text-slate-600"}`}>
                {p.estado}
              </span>
            </li>
          ))}
